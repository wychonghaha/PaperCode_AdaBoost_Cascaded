...
class RotationAdaBoostClassifier(BaseEnsemble):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for iboost in range(self.n_estimators):
            ...
            # print('第',iboost,'个RotationForest:',estimator_error)
    ...
    def _make_rotation_matrix(self, X, sample_weight, random_state):
        ...
        for column_inds, row_inds in zip(partition_iter, bootstrap_iter):
            # Make Weighted PCA
                  #row_inds[:, None] 样本行号 column_inds特征列号
    ...
    def _boost_real(self, iboost, X, y, sample_weight, random_state):
        ...
        if iboost == 0:
            self.classes_ = getattr(estimator, 'classes_', None) #获得类中的属性值
    ...
    def _boost_discrete(self, iboost, X, y, sample_weight, random_state):
        ...
        # Error fraction
        # estimator_pred=np.mean(np.equal(y_predict,y))
        # print('value:',estimator_pred)
    ...
    def decision_function(self, X):
        ...
        n_classes = self.n_classes_
        classes = self.classes_[:, np.newaxis] #[[0],[1],[2],[3]...[8],[9]]
        pred = None
